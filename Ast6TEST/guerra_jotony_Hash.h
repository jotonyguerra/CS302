...
Type2 &myHash<Type1, Type2>::operator[](Type1 key)
{
    ...
    //Insert entry if key is not found
    if (index < 0)
    {
        //check for if the list is full and resize
        if (table[x].amount == table[x].capacity)
        {
            //deep copy and increase capacity 
            ...
            //delete old list and allocate new list; 
            delete[] table[x].list;
            table[x].list = temp;
        }
        ...
        int j;
        entry *array = new entry[table[x].capacity];
        for (size_t i = 1; i < table[x].amount; i++)
        {
            array[i] = table[x].list[i];
            j = i - 1;
           ...
           // table[x].list[j + 1] = array;
            delete[] table[x].list;
            table[x].list = array;
        }
    }
    //redo search.... 
    right = table[x].amount;
    left = 0;
    //int mid = left + (right - left) / 2;
    if (right >= 1)
    {
